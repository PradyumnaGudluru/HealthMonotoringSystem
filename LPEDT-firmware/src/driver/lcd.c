...
void displayInit()
{
    ...
    //           set up a 1 second repeating soft timer and uncomment the following lines
	  ...
	  timer_response = sl_bt_system_set_soft_timer(32768,                 // Required time is 1 sec = 32768 ticks
	                                                0,                    // ble.c  '0'
	                                                0);                  // As timer gets calling again and again
	  if (timer_response != SL_STATUS_OK) {
	      LOG_ERROR("ERROR with timer_response %d", timer_response);
     }
...
void displayUpdate()
{
  ...
  // Students: Create the function gpioSetDisplayExtcomin() that will set
	...
	//   EXTCOMIN is a pin find it in Manual/ schematic.
