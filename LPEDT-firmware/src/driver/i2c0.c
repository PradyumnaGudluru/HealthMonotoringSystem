 ...
 *****************************************************************************/
static I2C_TransferReturn_TypeDef transferStatus; // A struct that keeps track of the transfer status
static I2C_TransferSeq_TypeDef transferSequence; // A struct that stores the transfer sequence that the data/command has to be transfered
...
void i2c0_Init()
{
  ...
  I2C_Config.sdaPin = 7,
  I2C_Config.portLocationScl = 10,
  I2C_Config.portLocationSda = 12,
 ...
 *****************************************************************************/
int i2c0_Write(uint8_t addr, uint8_t* wr_buff, uint8_t wr_buff_len)
{
  transferSequence.flags = I2C_FLAG_WRITE, // Write command
  transferSequence.addr = (addr<<1), // Slave address needs to be left shift by one bit
  transferSequence.buf[0].data = wr_buff, // Passing the pointer that has the command data stored
  transferSequence.buf[0].len = wr_buff_len; // Length of the command data
 ...
 *****************************************************************************/
int i2c0_Read(uint8_t addr, uint8_t* rd_buff, uint8_t rd_buff_len)
{
  transferSequence.flags = I2C_FLAG_READ, // Read command
  transferSequence.addr = (addr<<1), // Slave address needs to be left shift by one bit
  transferSequence.buf[0].data = rd_buff, // Passing the array that will store the incoming data
  transferSequence.buf[0].len = rd_buff_len; // Passing the size of the array
 ...
 * status of the transfer.
 *****************************************************************************/
int i2c0_Write_Read (uint8_t addr, uint8_t reg, uint8_t* read_data, size_t nbytes_read_data)
{
 ...
 * status of the transfer.
 *****************************************************************************/
int i2c0_Write_Write (uint8_t addr, uint8_t reg, uint8_t* write_data, size_t nbytes_write_data)
{
